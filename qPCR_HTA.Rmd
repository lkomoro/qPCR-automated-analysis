...
---
##*Note this .Rmd is a work in progress...transitioning from scripts...*
...
write.table(HK.genes.w.undet.df, file = "HK.genes.w.undet.csv", sep = ",", col.names = NA,
            qmethod = "double")
```
Alternatively, it like there''s something's a function tonormalize in Bioconductor, [script looks pretty simple] [2], but I haven't had time yet to look into it.
```{r BioC, echo=TRUE}
source("http://bioconductor.org/biocLite.R")
biocLite("qpcrNorm")
```

###Method One: if you just want to normalize to a reference gene *For Example, Let's say that G6PD is our HK gene*
...
w <- reshape(normalized.data1.short, 
             ...
             direction = "wide")
```
###Method 2: You already have the stability calculated, so you know what genes to use and want to calculate the normalization factor directly here
...
Example of normalizing with test data:
```{r test, echo=TRUE}
...
QC-after writing these functions and running them on test data, I pasted these values into spreadsheet with GeNorm formulas-confirmed exact same values) *Code in excel=(PRODUCT(range of ref gene values per each sample))^(1/COUNT(range of ref gene values per each sample))*
...
write.table(data3b, file = "data.norm.check2.csv", sep = ",", col.names = NA,
            qmethod = "double")
```
For, it was that 2 genes for a blank actually had Ct values (high 30's) so they were retained, but since the ref genes had been removed as NA's, there was nothing to match them too (see the qPCR spreadsheet for further notes)

So actually, we want to delete these cases so good to use 'data3b' for downstream
```{r clean, echo=TRUE}
